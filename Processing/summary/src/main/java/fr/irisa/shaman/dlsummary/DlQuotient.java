...
public class DlQuotient {
	...
	public DlQuotient(Properties _properties, String _dbname) {
		...
		String expsPath = System.getProperty("user.dir")
				+ "/"+properties.get("stats.output.prefix").toString()+properties.get("ontology.name").toString()+"/"
				+ "/DBMS/" + this.properties.getProperty("database.engine").toUpperCase().trim()
				+ "/" + this.properties.getProperty("database.size").toUpperCase().trim()
				+ "/Stats/";
	...
	public void init() {
		String engine_alias = this.engine.toLowerCase().trim();
		//Old way
		...
		//New way
		String user = this.properties.getProperty("database."+engine_alias+".user");
		String password = this.properties.getProperty("database."+engine_alias+".password");
		String host = this.properties.getProperty("database.host");
		String port = this.properties.getProperty("database."+engine_alias+".port");
	...
	public void createRole(int id) {
		...
		if(this.engine.contains("DB2")){
			System.out.println("Here 275----id: "+id);
			update(String.format("DROP TABLE IF EXISTS table%s%d ;\n", alias, id));
	...
	public void updateStats() {
		...
		try {
			...
			writer.write(String.format("Number of concepts: %d concept(s) \n", nbConcepts));
			writer.write(String.format("Number of roles: %d role(s) \n", nbRoles));
			writer.write(String.format("Number of empty concepts: %d concept(s) \n", emptyConcepts));
			writer.write(String.format("Number of empty roles: %d role(s) \n", emptyRoles));
			writer.write(String.format("Number of constants: %d constant(s) \n", nbConstants));
			writer.write(String.format("Number of classes: %d classe(s) \n", nbClasses));
			writer.write(String.format("Initial data base tuples: %d tuple(s) \n", dbTuples));
			writer.write(String.format("Summary tuples: %d tuple(s) \n", summaryTuples));
			writer.write(String.format("Compression ratio: %,.003f", 100.0*summaryTuples/dbTuples)+" % \n");
			writer.write(String.format("Time to create the summary: %d ms \n", summaryTime));
			writer.close();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		if (args.length == 0 || args.length == 1 || args.length == 2 ||args.length == 3 ) {
			...
			System.out.println("you need to provide a config file, a DBMS and a database SIZE. Ex java -jar dlssummary.jar config.properties");
			System.exit(0);
		}
...
//			System.out.println("Starting summary creation code.");
...
//		}
			...
			System.out.println("Computing summary with config file : " + args[0]);
			System.out.println("Computing summary using engine : " + args[1]);
			System.out.println("Computing summary for database size : " + args[2]);
			System.out.println("Computing summary for ontology name : " + args[3]);
			String propertiesPath = System.getProperty("user.dir") + "/" + args[0];
			...
			try {
				...
				prop.setProperty("database.engine", args[1].toUpperCase());
				prop.setProperty("database.size", args[2].toUpperCase());
				prop.setProperty("ontology.name", args[3].toUpperCase());
				System.out.println("database."+(args[1].toLowerCase().trim())+"."+(args[3].toLowerCase().trim())+"."+(args[2].toLowerCase().trim())+".name");
				String databaseName = prop.get("database."+(args[1].toLowerCase().trim())+"."+(args[3].toLowerCase().trim())+"."+(args[2].toLowerCase().trim())+".name").toString();
				System.out.println("Database name: "+databaseName);

				System.out.println(prop.get("database.engine").toString());

				DlQuotient dlquotient = new DlQuotient(prop, databaseName);
				dlquotient.computeQuotient();
				if(prop.get("database.summarize").toString().equals("true")){
					System.out.println("creating the summary ...");
					dlquotient.createDump();
			...
			System.out.println("Summary creation code completed.");
