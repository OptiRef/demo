...
public class QueryDL
{
	...
	public QueryDL(Properties _properties, String _dbname) {
		...
		this.NREF    = 0;
		this.NPRUN   = 0;
		this.dbsize  = properties.get("database.size").toString();
		this.properties = _properties;
		this.originalOutputDir = properties.get("exps.output").toString();
		...
		if(properties.get("current.base.directoryPath").toString().trim().length()>1){
			this.baseDirectory = properties.get("current.base.directoryPath").toString().trim();
		}
		...
		ontofile = "file:"+this.baseDirectory + "/"+properties.get("database.ontology").toString();
	...
	public QueryDL(Properties _properties, String datasetname, String datasetsize, String dbms, String refLang, String queryFile) {
		//Demo version
		this.TTOTAL  = 0;
		this.TREF    = 0;
		this.TPRUN   = 0;
		...
		this.NREF    = 0;
		this.NPRUN   = 0;
		this.dbsize  = datasetsize; //ex "1MT"

		this.properties = _properties;
		this.originalOutputDir = properties.get("stats.output.prefix").toString();
		//this.originalOutputDir = properties.get("exps.output").toString();
		this.queriesFile = queryFile; //(String) properties.get("database.queries");


		this.approach = refLang.toUpperCase();// (String) properties.get("reformulation.approach");
		this.debugger = properties.get("debugger.log_level").toString();
		this.nexp = Integer.parseInt(properties.get("exps.runs").toString());
		this.engine = dbms;//properties.get("database.engine").toString();
		...
		maxref = Integer.parseInt(properties.get("query.ref.max").toString());
		this.timeout = Integer.parseInt(properties.get("database.timeout").toString());
		ontofile = "file:"+this.baseDirectory + "/"+properties.get("database."+datasetname.toLowerCase()+".ontology").toString();

		//String databaseName = properties.get("ontology."+datasetname.toLowerCase()+"."+dbms.toLowerCase()+".basename").toString().trim()+""+datasetsize.substring(0,datasetsize.indexOf("M"));
		System.out.println();
		System.out.println("database."+dbms.toLowerCase()+"."+datasetname.toLowerCase()+"."+this.dbsize+".name");
		System.out.println();
		String databaseName = properties.get("database."+dbms.toLowerCase()+"."+datasetname.toLowerCase()+"."+this.dbsize.toLowerCase()+".name").toString();

		connector = new Connector(properties, databaseName, this.engine);
		connector.clean_explain_statement();
	...
	public int getUSCQTime(String qname) {
		int res = 0;
		String querypath    = this.baseDirectory + "/"+properties.get("compact.queryDir").toString();
	...
	public void loopDemo(boolean allApproaches) throws SQLException{
		//String queriesDirectory = properties.get("database.queryDir").toString();
		String queriesDirectory = properties.get("stats.output.prefix").toString()+"/Queries/Original/";
		File directoryPath = new File(this.baseDirectory + "/"+queriesDirectory);
		File filesList[] = directoryPath.listFiles();
		for(File file : filesList) {
			this.queriesFile = file.getAbsolutePath();
			if(allApproaches) {
				evalDemoAllApproaches();
			}else {
				evalDemo();
			}
	...
	public void evalDemoAllApproaches() throws SQLException{
		this.approach = "UCQ";
		evalDemo();
		this.approach = "USCQ";
		evalDemo();
	...
	public void evalDemo() throws SQLException{
		properties.setProperty("exps.stats", "false");
		properties.setProperty("database.pruning", "false");
		properties.setProperty("pruning.summary", "false");
		String compactDir = this.queriesFile.substring(0,this.queriesFile.lastIndexOf("/")).replace("Original", "Compact");
		properties.setProperty("compact.queryDir", compactDir);

		properties.setProperty("database.queries",this.queriesFile);

		properties.setProperty("query.savesubcq", "true");
		String savesubcqDir = this.originalOutputDir+properties.getProperty("ontology.name").toString()
				+"/Queries/Reformulations/"+this.approach+"/"+this.dbsize+"/";
		properties.setProperty("query.savesubcq.directoryPath", savesubcqDir);

		String queryName = this.queriesFile.substring(this.queriesFile.lastIndexOf("/")+1, this.queriesFile.indexOf("."));
		System.out.println("Before");
		System.out.println(properties.get("exps.output"));

		String newDir = this.originalOutputDir+properties.getProperty("ontology.name").toString()
				+"/DBMS/"+this.engine+"/"+this.dbsize
				+"/Queries/"+queryName+"/"+this.approach+"/";

		System.out.println("After");
		System.out.println(newDir);

		properties.setProperty("exps.output",newDir);

		eval();

		properties.setProperty("database.pruning", "true");

		eval();

		properties.setProperty("pruning.summary", "true");
	...
	public void  eval() throws SQLException{
		...
		System.out.println(properties.get("exps.output").toString());
		System.out.println("check?userdir");
		//System.out.println(System.getProperty("user.dir"));

		String out = this.baseDirectory + "/"+properties.get("exps.output").toString();
			...
			for(Integer r : ProgressBar.wrap(pExps, String.format("Running exps for %s...", qname))) {
				...
				if(r == 0) {
					...
					if(this.properties.get("query.store").equals("ALL")){
						String queryfilename = this.baseDirectory + "/"+properties.get("exps.output").toString();
						queryfilename += String.format("%s_%s.%s.sql", qname, this.dbname, this.approach);
						QueryUtils.storeQuery(sqlCode, queryfilename);
						queryfilename = this.baseDirectory + "/"+properties.get("exps.output").toString();
...
//				System.exit(0);
    ...
    public static void main( String[] args )
    {
    	if (args.length == 0) {
        	...
        	//We're in demo mode. Time to evaluate and compute everything.
        	//System.out.println("you need to provide a config file and a database name. Ex java -jar dlssummary.jar config.properties");
        	//Need to provide the following:
        	//  - config file
        	//  - Dataset name (ex: lubm150) // ontology name
        	// 	- Database size (ex: 1MT)
        	// 	- DBMS (ex: MySQL)
        	//  - Ref Language (ex: UCQ)
        	//  - Query file or file path (ex: Experiments/11-Oct-2022/LUBM/Queries/Original/)
        	//String propertiesPath = System.getProperty("user.dir") + "/" + args[0];
			String debugger;
			InputStream input;
			try {
				...
				Properties prop = new Properties();
				prop.load(input);
				prop.setProperty("database.engine", args[3]);
				prop.setProperty("database.size", args[2]);
				debugger = prop.get("debugger.log_level").toString();
				if(!debugger.equals("OFF")){
					System.out.println("Running exps with config file: " + args[0]);
					System.out.println("Running exps using database: " + args[3]);
					System.out.println("Using RDBMS: "+prop.get("database.engine"));
				}
				QueryDL querydl ;
//				if(prop.get("exps.method").toString().equals("theoric")) {
...
//				}else {
					...
					querydl = new QueryDL(prop, args[1], args[2],args[3],args[4], args[5]);
					if(args[5].endsWith("/")) {
		        		//Directory so loop
						if("ALL".equals(args[4])) {
							querydl.loopDemo(true);
						}else {
							querydl.loopDemo(false);
						}
